...
class TwitterRegisterRelationshipsJob < ApplicationJob
  ...
  def perform(twitter_screen_name_or_id, layer_count = '0')
    ...
    # 一意のユーザーIDを選択
    twitter_target_ids = (twitter_friend_ids | twitter_follower_ids).reject { |twitter_id| TwitterUser.exists?(twitter_id: twitter_id) }
    ...
    ActiveRecord::Base.transaction do
      ...
      TwitterRelationship.where(follower_id: me.id).where.not(followed_id: [twitter_friend_ids]).each(&:destroy!)
    ...
    ActiveRecord::Base.transaction do
      ...
      TwitterRelationship.where(followed_id: me.id).where.not(follower_id: [twitter_follower_ids]).each(&:destroy!)
    ...
    if layer_count.positive?
      (twitter_friend_ids | twitter_follower_ids).each do |id|
